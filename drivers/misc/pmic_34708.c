...
int pmic_set_chg_current(pmic_charge_type chg_value)
{
    ...
    switch (chg_type) {
      case pmic_charge_none: curr = 0;
      case pmic_charge_host: curr = CHARGING_HOST;
      case pmic_charge_usb_high: curr = CHARGING_USB_HIGH;
      case pmic_charge_third_party: curr = CHARGING_THIRD_PARTY;
      case pmic_charge_wall: curr = CHARGING_WALL;
    }
...
int pmic_charge_restart(void)
{
    ...
    if (sense_0 & (CHGCURRS | INT_USBDETS)) {
...
int pmic_set_alarm(unsigned int secs_from_now) 
{
    ...
    /* Clear RTC alarm */
    ret = pmic_wor_reg(MC34708_REG_INT_STATUS1, TOD, TOD);
    if (!ret)
	return ret;

    /* Clear RTC alarm interrupt mask */
    ret = pmic_wor_reg(MC34708_REG_INT_MASK1, 0, TOD);
    if (!ret)
	return ret;
