...
// 64k MMC writes are optimal
#define MAX_MMC_TRANSFER	65536	

#define BUFFER_LEN MAX_MMC_TRANSFER
...
static void fastboot_parse_cmd(char *cmdbuf) 
{
	...
	while (len > 0) {
	    pktsize = MIN(len, MAX_MMC_TRANSFER);
	...
	/* Erase the entire flash (clear to 0xFFs) */
	flash_addr = 0;
	...
	eraselen = mmc->capacity;
#else
	printf("FAILunsupported!\n");
	goto out;
#endif

	printf("erasing main flash, %lld bytes\n", eraselen);
	while (eraselen > 0) {
	    // 64k MMC writes are optimal
	    pktsize = MIN(eraselen, BUFFER_LEN);
	...
	eraselen = mmc->boot_size_mult * (128 * 1024);

	printf("erasing boot partition, %lld bytes\n", eraselen);
	while (eraselen > 0) {
	    // 64k MMC writes are optimal
	    pktsize = MIN(eraselen, BUFFER_LEN);
