 ...
 */
#define UARTDM_SR_RX_READY		 BIT(0) /* Receiver FIFO has data */
#define UARTDM_SR_TX_READY		 BIT(2) /* Transmitter FIFO has space */
#define UARTDM_SR_TX_EMPTY		 BIT(3) /* Transmitter underrun */

#define UARTDM_CR_RX_ENABLE		 BIT(0) /* Enable receiver */
#define UARTDM_CR_TX_ENABLE		 BIT(2) /* Enable transmitter */
...
static bool msm_serial_flush_rx(struct msm_serial_data *priv)
{
	...
	priv->rx_count = min_t(u32, count, sizeof(priv->rx_word));
...
static int msm_serial_getc(struct udevice *dev)
{
	...
	if (priv->regs->single_char) {
		if (!(readl(priv->base + priv->regs->sr) & UARTDM_SR_RX_READY))
			return -EAGAIN;

		return readl(priv->base + priv->regs->rf) & UARTDM_RF_CHAR;
	}
...
static int msm_serial_putc(struct udevice *dev, const char ch)
{
	...
	if (!priv->regs->single_char && priv->regs->ncf_tx) {
...
static int calc_csr_bitrate(struct msm_serial_data *priv)
{
	...
	for (; i >= 0; i--) {
		...
		if (x == baud)
			/* Duplicate the configuration for RX as the lower nybble only configures TX. */
			return i + (i << 4);
	}
...
static void uart_dm_init(struct msm_serial_data *priv)
{
	...
	if (bitrate < 0) {
		...
		/* This is the value used on MSM8916 for the hardcoded APQ8016 clock rate. */
		bitrate = 0xCC;
	}

	writel(bitrate, priv->base + priv->regs->csr);
	writel(priv->regs->single_char ? UARTDM_MR1_RX_RDY_CTL : 0,
	...
	writel(priv->regs->single_char ? UARTDM_DMEN_TXRX_SC_ENABLE : 0,
	...
	if (!priv->regs->single_char && priv->regs->dmrx)
		msm_serial_reset_stale(priv);
}

static int msm_serial_remove(struct udevice *dev)
{
	struct msm_serial_data *priv = dev_get_priv(dev);
	...
	writel(0, priv->base + priv->regs->dmen);
	writel(UARTDM_CR_CMD_RESET_RX, priv->base + priv->regs->cr);
	writel(UARTDM_CR_CMD_RESET_TX, priv->base + priv->regs->cr);
	writel(UARTDM_CR_CMD_RESET_ERR, priv->base + priv->regs->cr);
	writel(UARTDM_CR_RX_ENABLE, priv->base + priv->regs->cr);
