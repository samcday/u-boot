...
extern void mddr_init_1_0(void);
...
int dram_init(void)
{
	...
	if (board) {
		...
		if (is_soc_rev(CHIP_REV_1_0) == 0) {
		    printf("Initing mx508 1.0 MDDR memory\n");
		    mddr_init_1_0();
		} else {
		    printf("Initing MDDR memory\n");
		    mddr_init();
#ifdef CONFIG_ZQ_CALIB		    
		    printf("ZQ calibration complete: 0x128=0x%08x 0x12C=0x%08x\n", readl(DATABAHN_BASE_ADDR + 0x128), readl(DATABAHN_BASE_ADDR + 0x12C));
#endif
		}
#endif
		return 0;
		...
		if (strncmp(board->id, BOARD_ID_YOSHIME1, 5) == 0 || 
		    strncmp(board->id, BOARD_ID_CELESTE_WFO, 3) == 0 ||
		    strncmp(board->id, BOARD_ID_ICEWINE, 3) == 0) {
		    lpddr2_init(2);
		} else {
		    lpddr2_init(1);
		}
