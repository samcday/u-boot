...
#
# Entry-type module for legacy Qualcomm Android boot images
...
class Entry_android_boot_qcom_legacy(Entry_section):
    """Legacy Qualcomm Android boot image

    This creates an old Android boot image with Qualcomm's legacy QCDT extension.
    The QCDT payload is placed after the kernel, ramdisk and optional second
    stage, and the old dt_size/unused header fields are set to the QCDT size and
    zero respectively.

    Properties / Entry arguments:
        - page-size: Image page size, defaults to 2048
        - kernel-addr: Kernel load address, defaults to 0x80008000
        - ramdisk-addr: Ramdisk load address, defaults to 0x81000000
        - second-addr: Second-stage load address, defaults to 0x80f00000
        - tags-addr: ATAGS/FDT address, defaults to 0x80000100
    ...
    This entry uses the following subnodes:
        ...
        - ramdisk: optional section containing a ramdisk, defaults to one NUL byte
        - second: optional section containing a second-stage payload
        - qcdt: qcom-qcdt entry containing the Qualcomm DT table
    ...
    def ReadNode(self):
        ...
        self.second_addr = fdt_util.GetInt(self._node, 'second-addr',
                                           0x80f00000)
        self.tags_addr = fdt_util.GetInt(self._node, 'tags-addr', 0x80000100)
    ...
    @staticmethod
    def _BootId(kernel, ramdisk, second, qcdt):
        digest = hashlib.sha1()
        for data in (kernel, ramdisk, second, qcdt):
            digest.update(data)
    ...
    def BuildSectionData(self, required):
        ...
        qcdt = self._GetEntryData('qcdt', required)
        if qcdt is None:
            return None

        ramdisk = self._GetEntryData('ramdisk', required, b'\0')
        if ramdisk is None:
            return None
        ...
        image_id = self._BootId(kernel, ramdisk, second, qcdt)
        header = struct.pack('<8s10I16s512s32s', BOOT_MAGIC,
                             ...
                             len(qcdt), 0, boot_name, cmdline, image_id)
        ...
        image += _pad(qcdt, self.page_size)
