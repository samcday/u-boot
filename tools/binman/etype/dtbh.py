...
DTBH_MAGIC = b'DTBH'
# v2 is what dtbTool-exynos emits and what tested Samsung S-BOOT loaders accept.
DTBH_VERSION = 2
...
# S-BOOT appears to ignore this field; dtbTool-exynos emits 0x20 so we do too.
...
class Entry_dtbh(Entry_android_vendor_dt_table):
    ...
    def BuildSectionData(self, required):
        ...
        size = (DTBH_HEADER_SIZE + len(dtbs) * DTBH_RECORD_SIZE +
                DTBH_EOT_SIZE)
        header_size = tools.align(size, page_size)
        ...
        return tools.pad_align(dtbh, page_size) + bytes(payloads)
