...
'  fdtmap           90   243  fdtmap          90',
    ...
    def testQcdtBadDtbName(self):
        """Test that QCDT rejects record subnodes not named dtb-*"""
        with self.assertRaises(ValueError) as exc:
            self._DoReadFile('qcdt_bad_dtb_name.dts')
        self.assertIn("subnode 'not-a-dtb': must be named 'dtb-*'",
                      str(exc.exception))
    ...
    def testQcdtPageSizeTooSmol(self):
        """Test that QCDT rejects page sizes smaller than its header"""
        with self.assertRaises(ValueError) as exc:
            self._DoReadFile('qcdt_pagesize_too_smol.dts')
        self.assertIn("page-size must fit the QCDT header",
                      str(exc.exception))
