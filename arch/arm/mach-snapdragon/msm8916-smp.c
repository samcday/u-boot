// SPDX-License-Identifier: GPL-2.0+
/*
 * On MSM8916 devices that lack a PSCI implementation, firing up the secondary
 * cores requires a call to TZ to set the boot address, and some poking of APCS
 * registers.
 *
 * This code was mostly adapted from arch/arm/mach-qcom/platsmp.c in the kernel:
 *  Copyright (C) 2002 ARM Ltd.
 *  All Rights Reserved
 *  Copyright (c) 2010, Code Aurora Forum. All rights reserved.
 *  Copyright (c) 2014 The Linux Foundation. All rights reserved.
 *
 * Some parts were adapted from, or referenced, lk2nd/smp/cortex-a.c in lk2nd:
 *  Copyright (c) 2021-2022, Stephan Gerhold <stephan@gerhold.net>
 */

#include <asm/io.h>
#include <asm/spin_table.h>
#include <asm/system.h>
#include <cpu_func.h>
#include <dm/ofnode.h>
#include <fdtdec.h>
#include <firmware/qcom/scm.h>
#include <linux/arm-smccc.h>
#include <linux/delay.h>

#define APCS_CPU_PWR_CTL 0x04
#define CORE_PWRD_UP BIT(7)
#define COREPOR_RST BIT(5)
#define CORE_RST BIT(4)
#define CORE_MEM_HS BIT(3)
#define CORE_MEM_CLAMP BIT(1)
#define CLAMP BIT(0)

#define APC_PWR_GATE_CTL 0x14
#define GDHS_CNT_SHIFT 24
#define GDHS_EN BIT(0)

#define PD_CELLS "#power-domain-cells"
#define PD_DOMAINS "power-domains"
#define PD_NAMES "power-domain-names"

#define PD_PROP_MAX_LEN 128

static int fdt_del_range(void *fdt, int node, const char *prop, int offset,
			 int len)
{
	const char *data;
	char tmp[PD_PROP_MAX_LEN];
	int new_len, prop_len;

	data = fdt_getprop(fdt, node, prop, &prop_len);
	if (!data)
		return prop_len;

	if (offset < 0 || len <= 0 || offset + len > prop_len)
		return -FDT_ERR_BADVALUE;

	if (len == prop_len)
		return fdt_delprop(fdt, node, prop);

	new_len = prop_len - len;
	if (new_len > sizeof(tmp))
		return -FDT_ERR_NOSPACE;

	memcpy(tmp, data, offset);
	memcpy(tmp + offset, data + offset + len, prop_len - offset - len);

	return fdt_setprop(fdt, node, prop, tmp, new_len);
}

static int remove_psci_power_domain(void *fdt, int cpu)
{
	int domain_len = 0, domain_offset = 0;
	int i, index, name_offset, ret;
	struct fdtdec_phandle_args args;
	const char *name, *names;

	index = fdt_stringlist_search(fdt, cpu, PD_NAMES, "psci");
	if (index == -FDT_ERR_NOTFOUND)
		return 0;
	if (index < 0)
		return index;

	names = fdt_getprop(fdt, cpu, PD_NAMES, NULL);
	name = fdt_stringlist_get(fdt, cpu, PD_NAMES, index, &ret);
	if (!names || !name)
		return ret;
	name_offset = name - names;

	for (i = 0; i <= index; i++) {
		ret = fdtdec_parse_phandle_with_args(fdt, cpu, PD_DOMAINS,
						     PD_CELLS, 0, i, &args);
		if (ret)
			return ret;

		domain_len = (args.args_count + 1) * sizeof(fdt32_t);
		if (i < index)
			domain_offset += domain_len;
	}

	ret = fdt_del_range(fdt, cpu, PD_DOMAINS, domain_offset, domain_len);
	if (ret)
		return ret;

	return fdt_del_range(fdt, cpu, PD_NAMES, name_offset, name_len + 1);
}

void qcom_msm8916_smp_fixup(void *fdt)
{
	int cpus, cpu, ret;
	uint reg;
	const char *prop;

	/*
	 * If spin-table support is enabled, make sure any CPU nodes with a
	 * PSCI enable-method are updated to spin-table. Further, any PSCI
	 * power-domain entries are removed.
	 */

	cpus = fdt_path_offset(fdt, "/cpus");
	if (cpus < 0)
		return;

	fdt_for_each_subnode(cpu, fdt, cpus) {
		prop = fdt_getprop(fdt, cpu, "device_type", NULL);
		if (!prop || strcmp(prop, "cpu"))
			continue;

		prop = fdt_getprop(fdt, cpu, "enable-method", NULL);
		if (!prop || strcmp(prop, "psci"))
			continue;

		reg = fdtdec_get_uint(fdt, cpu, "reg", 0);
		ret = fdt_setprop_string(fdt, cpu, "enable-method",
					 "spin-table");
		if (ret)
			log_err("Failed to set CPU%d enable-method to 'spin-table', this CPU will not be available to booted OS! %d\n",
				reg, ret);

		log_info("Patched CPU%d enable-method to 'spin-table'\n", reg);

		ret = remove_psci_power_domain(fdt, cpu);
		if (ret)
			log_err("Failed to remove CPU%d PSCI power-domain: %d\n",
				reg, ret);
	}
}

static void qcom_boot_cortex_a53(phys_addr_t acc_base)
{
	u32 reg_val;

	/* Put the CPU into reset. */
	reg_val = CORE_RST | COREPOR_RST | CLAMP | CORE_MEM_CLAMP;
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();

	/* Turn on the GDHS and set the GDHS_CNT to 16 XO clock cycles */
	writel(GDHS_EN | (0x10 << GDHS_CNT_SHIFT), acc_base + APC_PWR_GATE_CTL);
	/* Wait for the GDHS to settle */
	dsb();
	udelay(2);

	reg_val &= ~CORE_MEM_CLAMP;
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();

	reg_val |= CORE_MEM_HS;
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();
	udelay(2);

	reg_val &= ~CLAMP;
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();
	udelay(2);

	/* Release CPU out of reset and bring it to life. */
	reg_val &= ~(CORE_RST | COREPOR_RST);
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();

	reg_val |= CORE_PWRD_UP;
	writel(reg_val, acc_base + APCS_CPU_PWR_CTL);
	dsb();
}

static int qcom_scm_set_boot_addr_mc(void *entry, unsigned int flags)
{
	struct qcom_scm_desc desc = {
		.svc = QCOM_SCM_SVC_BOOT,
		.cmd = QCOM_SCM_BOOT_SET_ADDR_MC,
		.owner = ARM_SMCCC_OWNER_SIP,
		.arginfo = QCOM_SCM_ARGS(6),
		.args = {
			(u64)entry,
			/* Apply to all CPUs in all affinity levels */
			~0ULL, ~0ULL, ~0ULL, ~0ULL,
			flags,
		},
	};

	if (!qcom_scm_is_call_available(desc.svc, desc.cmd))
		return -EOPNOTSUPP;

	return qcom_scm_call(&desc, NULL);
}

static bool boot_addr_set;

int spin_table_boot_cpu(void *fdt, int cpu_offset)
{
	struct fdtdec_phandle_args acc;
	u32 mpidr_aff, acc_base, reg;
	int ret;

	reg = fdtdec_get_uint(fdt, cpu_offset, "reg", 0);

	if (fdt_node_check_compatible(fdt, cpu_offset, "arm,cortex-a53")) {
		log_warning("CPU%d is not arm,cortex-a53 compatible\n", reg);
		return -EINVAL;
	}

	mpidr_aff = read_mpidr() & 0xffffff;

	if (reg == mpidr_aff) {
		debug("Skipping boot of current CPU%d\n", reg);
		return 0;
	}

	ret = fdtdec_parse_phandle_with_args(fdt, cpu_offset, "qcom,acc",
					     NULL, 0, 0, &acc);
	if (ret) {
		log_err("Failed to parse qcom,acc phandle: %d\n", reg);
		return ret;
	}

	acc_base = fdtdec_get_addr_size_auto_noparent(fdt, acc.node, "reg", 0,
						      NULL, true);
	if (!acc_base) {
		log_err("Failed to parse qcom,acc regbase\n");
		return -EINVAL;
	}

	log_info("Booting CPU%d @ 0x%x\n", reg, acc_base);

	if (!boot_addr_set) {
		debug("Setting CPU boot address to 0x%llx\n",
		      (phys_addr_t)&spin_table_reserve_begin);
		ret = qcom_scm_set_boot_addr_mc(&spin_table_reserve_begin,
						QCOM_SCM_BOOT_MC_FLAG_AARCH64 |
						QCOM_SCM_BOOT_MC_FLAG_COLDBOOT);
		if (ret) {
			log_err("Failed to set CPU boot addr: %d\n", ret);
			return ret;
		}

		boot_addr_set = true;
	}

	qcom_boot_cortex_a53(acc_base);
	return 0;
}
